...
# 读取 CSV 数据
data = pd.read_csv("your_data.csv") # to edit
...
# 将特征转换为字符串格式，作为模型的输入
data["input"] = features.apply(lambda row: " ".join(map(str, row)), axis=1)
# checkt The result
...
def process_func(example):
    ...
    instruction = tokenizer(
        f"<|im_start|
        system\nYou are an expert in predicting antibiotic resistance based on patient data. Please output the prediction results.<|im_end|>\n<|im_start|>user\n{feature_str}<|im_end|>\n<|im_start|>assistant\n",
        add_special_tokens=False,
    ) # to edit
...
txt_add = "./output/"+ antimicrobial +"_"+str(feature_num)
